...
function startTangoQuiz() {
    ...
    if (mode !== 'all') {
        const selected = [...document.querySelectorAll(`#tango-filter-list input[data-filter="${mode}"]:checked`)].map(i => i.value);
...
window.restartGame = function () {
    if (lastQuizContext === 'tango') {
        if (!tangoQuizState.questions || tangoQuizState.questions.length === 0) {
            showScreen('tango-hub');
            return;
        }
        // Re-shuffle the same pool of questions for replay.
        tangoQuizState.questions = shuffleArray(tangoQuizState.questions);
        tangoQuizState.index = 0;
        tangoQuizState.score = 0;
        tangoQuizState.correct = 0;
        tangoQuizState.wrong = 0;
        showScreen('tango-quiz');
        renderTangoQuestion();
        return;
    }
